...
Comix 是一个用 Rust 编写的静态类型、解释型脚本语言，支持整数、浮点数、布尔、字符串和数组类型，具有变量作用域、控制流、函数定义、递归调用等特性。此项目已实现较为完整的语言核心，并提供交互式 REPL、脚本文件执行以及图形界面（GUI）。

## 特性概览

- **基本类型**：整数 (`int`)、浮点数 (`float`)、布尔 (`bool`)、字符串 (`string`)、数组 (`array<T>`)
- **运算符**：算术（`+ - * /`）、比较（`= < > <= >= !=`）、逻辑（`and or not`，支持短路求值）、负号（`-`）
- **变量声明与赋值**：`int x, y;` `x := 10;` 数组赋值 `arr := [1, 2, 3];`
- **控制流**：`if`/`else`、`while`、`for`（`for var := start to end do { ... }`）
- **函数**：`func name(params) -> ret_type { ... }`，支持参数、返回值、递归、无返回值函数
- **内置输出**：`print expr;`
- **数组操作**：索引访问 `arr[0]`，索引赋值 `arr[1] := 5`，数组字面量 `[1, 2, 3]`（支持空数组 `[]`）
- **注释**：单行注释 `// ...`
...
numbers[1] := 99;
print numbers[1];     // 输出 99
print numbers;        // 输出 [10, 99, 30]
